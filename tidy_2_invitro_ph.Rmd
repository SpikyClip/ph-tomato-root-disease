...
fusa_tidy_5 = 
    ...
    mutate(
        dimension = recode(dimension, W = "x", L = "y"),
        day = str_extract(day, r"(\d+)"),
        # Avoided regex here as the 'control' level was not matching
        ph = replace(ph, ph == "5.5 (Control)", "control")
    ) %>%
    add_column(trial = 5, start_date = "19/09/2019") %>%
    pivot_wider(
        id_cols = c(start_date, trial, species, ph, replicate, day, comment),
        names_from = dimension,
        values_from = length_cm,
...
pyth_tidy_6 =
    ...
    add_column(trial = 5, start_date = "25/09/2019") %>%
    pivot_wider(
        id_cols = c(start_date, trial, species, ph, replicate, day, comment),
        names_from = dimension,
        values_from = length_cm,
